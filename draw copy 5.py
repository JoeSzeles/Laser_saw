...
def save_gcode():
    ...
    for line in lines:
        ...
        gcode_lines.append(f"G1 X{x1} Y{y1} F3000")  # Move to starting position
        gcode_lines.append("M3 S1000")  # Turn laser on
        gcode_lines.append(f"G1 X{x1} Y{y1} F3000")  # Move to starting position (redundant)
...
def Engrave():
    ...
    for line in lines:
        ...
        gcode_lines.append(f"G1 X{x1} Y{y1} F{speed} S{power}")

        # Turn laser on
        gcode_lines.append(f"M3 S{power}")

        # Move to starting position (redundant)
        gcode_lines.append(f"G1 X{x1} Y{y1} F{speed}")
